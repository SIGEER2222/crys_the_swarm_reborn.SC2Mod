param(
    ...
    [string]$SummaryPath = "references\official-artanis-import-summary.tsv"
)
...
if ([string]::IsNullOrWhiteSpace($TargetGameDataRoot)) {
    $scenarioRoot = Get-ChildItem -LiteralPath $projectRoot.FullName -Directory | Where-Object {
        Test-Path -LiteralPath (Join-Path $_.FullName "Mods\XM\XMArtanis.SC2Mod")
    } | Select-Object -First 1
    if (-not $scenarioRoot) {
        throw "Unable to locate scenario root containing Mods\XM\XMArtanis.SC2Mod under $($projectRoot.FullName)"
    }
    $TargetGameDataRoot = Join-Path $scenarioRoot.FullName "Mods\XM\XMArtanis.SC2Mod\Base.SC2Data\GameData"
}
...
$seedIds = @(
    "Artanis", "ProtossArtanis", "ArtanisCommander",
    "SoACasterArtanis", "SoACaster", "SOAPylonPower",
    "SOAOrbitalStrike", "SOAOrbitalStrikeActivate", "SOAOrbitalStrikeTargetingDummy", "SOAOrbitalStrikeExecute", "SOAOrbitalStrikeUpgrade",
    "SOASuperShield", "SOAHeroicShield", "SOARecallOnDeath", "SOAThermalLanceExecute", "SOAStrafeAttack", "SOAStrafeAttackActivate",
    "SOAStrafeAttackExecute", "SOAStrafeAttackTargetingDummy", "SOASolarBombardment", "SolarBombardment",
    "ArtanisUnlockHeroicShield", "ArtanisUnlockWarpGateCharges", "ArtanisUnlockWarpTech", "ArtanisSOAReinforcements",
    "ArtanisSOAThermalLance", "ArtanisSOAHeroicShield", "ArtanisSOASuperShield", "ArtanisSOATimeStop",
    "Probe", "Nexus", "Pylon", "Assimilator", "Gateway", "WarpGate", "CyberneticsCore", "Forge",
    "TwilightCouncil", "RoboticsFacility", "RoboticsFacilityWarp", "RoboticsBay", "Stargate", "StargateWarp",
    "FleetBeacon", "TemplarArchive", "PhotonCannon", "ShieldBattery",
    "Zealot", "ZealotAiur", "StalkerAiur", "Dragoon", "HighTemplar", "Archon", "ImmortalAiur",
    "Observer", "PhoenixAiur", "Reaver", "Tempest", "Carrier", "CarrierAiur", "ArbiterMP",
    "StalkerResearchDragoonHealth", "StalkerResearchDragoonRange", "ReaverIncreasedScarabSplashRadius",
    "ReaverIncreasedScarabCount", "ArtanisTempestDisintegration", "ArtanisUnlockHighArchon",
    "CommanderPrestigeArtanisOrbitalStrikes", "CommanderPrestigeArtanisOrbitalStrikesShared",
    "CommanderPrestigeArtanisCombatAbilities", "CommanderPrestigeArtanisPowerField",
    ...
    "MasteryArtanisSolarBombardment", "MasteryArtanisChronoBoost",
    "MasteryArtanisGuardianShell", "MasteryArtanisGuardianShellHeal",
    "MasteryArtanisStartingResources", "MasteryArtanisStartingAndMaxSoAEnergy", "MasteryArtanisEnergyRegenCooldown"
)

$seedPattern = "^(Artanis|CommanderPrestigeArtanis|MasteryArtanis|HaveMasteryArtanis|HaveArtanis|SoACasterArtanis|SOAOrbitalStrike|SOASuperShield|SOAHeroicShield|SOAPylonPower|SOAThermalLance|SOAStrafeAttack|SOARecallOnDeath|SolarBombardment|ZealotAiur|StalkerAiur|ImmortalAiur|PhoenixAiur|CarrierAiur|Reaver|Dragoon)"
$seedPatternEnd = "(Artanis|Aiur)$"
...
foreach ($id in @($selected)) {
    if ($idToSourceFile[$id] -eq "commanderdata.xml" -and $id -ne "Artanis") {
...
foreach ($file in ($sourceDocs.Keys | Sort-Object)) {
    ...
    foreach ($node in $currentSourceDoc.Catalog.ChildNodes) {
        ...
        if ($file -eq "commanderdata.xml" -and $id -ne "Artanis") { continue }
...
Write-Host "Imported $($summaryRows.Count - 1) official Artanis-related catalog objects."
