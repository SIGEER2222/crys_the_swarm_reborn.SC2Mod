param(
    ...
    [string]$SummaryPath = "references\official-kerrigan-import-summary.tsv"
)
...
if ([string]::IsNullOrWhiteSpace($TargetGameDataRoot)) {
    ...
    $TargetGameDataRoot = Join-Path $scenarioRoot.FullName "Mods\XM\XMKerrigan.SC2Mod\Base.SC2Data\GameData"
}
...
$seedIds = @(
    "Kerrigan", "K5Kerrigan", "K5KerriganBurrowed", "CoopCasterKerrigan",
    "PrimalSlash", "MindBolt", "PsiStrike", "PsiStrikeWalk", "PsionicLift", "KerriganChainLightning",
    "KerriganVoidCoopEconDrop", "KerriganVoidCoopEconDrop2", "KerriganSpawnBanelings",
    "KerriganAssimilationAura", "KerriganCrushingGrip", "KerriganLeapingStrike",
    "VoidCoopHeroicFortitude", "K5HeroicFortitude", "KerriganPrimalRage",
    "KerriganLevel02", "KerriganLevel03", "KerriganLevel04", "KerriganLevel05", "KerriganLevel06",
    "KerriganLevel07", "KerriganLevel08", "KerriganLevel09", "KerriganLevel10", "KerriganLevel11",
    "KerriganLevel12", "KerriganLevel13", "KerriganLevel14", "KerriganLevel15",
    "CommanderPrestigeKerriganAbilities", "CommanderPrestigeKerriganAssimilationAura",
    "CommanderPrestigeKerriganAssimilationAuraShared", "CommanderPrestigeKerriganCreep",
    "HaveMasteryKerriganAutoAttackDamage", "HaveMasteryKerriganArmyGasCost",
    "HaveMasteryKerriganEnergyRegen", "HaveMasteryKerriganImmobilizationWaveDamage",
    "HaveMasteryKerriganResearchReductionCost", "HaveMasteryKerriganPrimarySpeedDamage",
    "NoKerriganInstantReviveCooldown", "IsK5Kerrigan", "IsK5KerriganBurrowed", "IsNotK5Kerrigan",
    "Hatchery", "Drone", "Overlord", "Larva", "Queen", "Zergling", "Hydralisk", "Mutalisk", "Ultralisk",
    "Baneling", "BanelingNest", "SpawningPool", "HydraliskDen", "Spire", "UltraliskCavern", "EvolutionChamber", "Extractor", "Lair", "Hive"
)

$seedPattern = "^(Kerrigan|K5Kerrigan|CommanderPrestigeKerrigan|HaveMasteryKerrigan|NoKerrigan|IsK5Kerrigan|IsNotK5Kerrigan|PrimalSlash|MindBolt|PsiStrike|PsionicLift)"
$seedPatternEnd = "(Kerrigan|K5Kerrigan)$"
$skipFiles = @(
...
foreach ($id in @($selected)) {
    if ($idToSourceFile[$id] -eq "commanderdata.xml" -and $id -ne "Kerrigan") {
...
foreach ($file in ($sourceDocs.Keys | Sort-Object)) {
    ...
    foreach ($node in $currentSourceDoc.Catalog.ChildNodes) {
        ...
        if ($file -eq "commanderdata.xml" -and $id -ne "Kerrigan") {
            continue
        }
...
if ($idsByTargetFile.ContainsKey("userdata.xml")) {
    $userDoc = $sourceDocs["userdata.xml"]
    foreach ($node in $userDoc.Catalog.ChildNodes) {
        if ($node.NodeType -ne "Element" -or -not $node.id) {
            continue
        }
        if ([string]$node.id -notmatch "^CampaignPerk$") {
            continue
        }
        if (-not ($idsByTargetFile["userdata.xml"] | Where-Object { $_.id -eq "CampaignPerk" })) {
            $idsByTargetFile["userdata.xml"].Add($node)
        }
    }
}
...
Write-Host "Imported $($summaryRows.Count - 1) official Kerrigan-related catalog objects."
