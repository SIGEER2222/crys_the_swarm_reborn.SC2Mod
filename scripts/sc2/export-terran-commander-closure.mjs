...
const outputDir = path.join(repoRoot, 'docs/newdocs/指挥官细化/人族闭包');
const outputMarkdownPath = path.join(outputDir, '人族指挥官完整闭包-2026-06-03.md');
const outputJsonPath = path.join(outputDir, 'terran-commander-closure.json');

const terranCommanders = [
  { id: 'Raynor', zh: '雷诺' },
  { id: 'Swann', zh: '斯旺' },
  { id: 'Horner', zh: '霍纳与汉' },
  { id: 'Nova', zh: '诺娃' },
...
const defaultAbilityIds = new Set([
  'attack',
  'BuildInProgress',
  'BuildInProgressTerran',
  'halt',
  'move',
  'stop',
]);

const workerBuilderUnitIds = new Set([
  'SCV',
  'HHSCV',
  'KelMorianWorker',
  'SCVMengsk',
...
const closure = {
  ...
  commanders: terranCommanders.map((commander) => buildCommanderClosure(commander)),
};
...
console.log(`已输出人族指挥官闭包 Markdown：${normalizePath(path.relative(repoRoot, outputMarkdownPath))}`);
console.log(`已输出人族指挥官闭包 JSON：${normalizePath(path.relative(repoRoot, outputJsonPath))}`);
...
function buildCommanderClosure(commanderInfo) {
  ...
  identity.buildingTargetIds = buildBuildingTargetIdSet(buildings);
  ...
  const workerBuildCommands = buildWorkerBuildCommands({ unitClosures, context });
...
function buildWorkerBuildCommands({ unitClosures, context }) {
  const records = [];
  const seen = new Set();

  for (const unit of unitClosures) {
    const unitIds = [unit.id, unit.unit_id, ...(unit.resolved_unit_ids || [])].filter(Boolean);
    if (!unitIds.some((id) => workerBuilderUnitIds.has(id))) {
      continue;
    }

    for (const ability of unit.abilities.accepted) {
      if (!isWorkerBuildAbility(ability)) {
        continue;
      }

      const targetUnitId = ability.raw?.command?.unit || inferWorkerBuildTargetFromFace(ability.face);
      const targetCanonicalId = context.identity.buildingCanonicalIdByAnyId.get(targetUnitId) || targetUnitId;
      const key = [unit.id, ability.abil_cmd, ability.face, targetUnitId].join('|');
      if (seen.has(key)) {
        continue;
      }
      seen.add(key);
      ...
      records.push({
        worker_id: unit.id,
        worker_unit_id: unit.unit_id,
        worker_name: unit.name,
        face: ability.face,
        abil_cmd: ability.abil_cmd,
        ability_id: ability.ability_id,
        command_index: ability.command_index,
        requirements: ability.requirements,
        target_unit_id: targetUnitId,
        target_canonical_id: targetCanonicalId,
        target_name: context.identity.nameByCanonicalId.get(targetCanonicalId) || '',
        target_in_official_buildings: hasAnyId(context.identity.buildingTargetIds, [targetUnitId, targetCanonicalId]),
        target_in_official_units: hasAnyId(context.identity.primaryIdsByGroup.units, [targetUnitId, targetCanonicalId]),
        target_in_official_roster: hasAnyId(context.identity.primaryIdsByGroup.roster, [targetUnitId, targetCanonicalId]),
        raw: ability.raw,
      });
    }
  }
  ...
  return records.sort((a, b) =>
    naturalSort(`${a.worker_id}|${a.ability_id}|${a.command_index}|${a.face}`, `${b.worker_id}|${b.ability_id}|${b.command_index}|${b.face}`),
  );
}

function inferWorkerBuildTargetFromFace(face) {
  const value = String(face || '').trim();
  if (!value || value === '-' || /Locked$/i.test(value) || /^(Halt|Cancel|Stop)$/i.test(value)) {
    return '';
  }
...
function hasAnyId(idSet, ids) {
  return ids.filter(Boolean).some((id) => idSet.has(id));
}

function isWorkerBuildAbility(ability) {
  if (!ability.abil_cmd || !ability.ability_id || ability.command_index === 'Halt') {
    return false;
  }

  if (/^CAbilBuild/.test(ability.raw?.kind || '')) {
    return true;
  }

  return /(?:TerranBuild|HHBuild|KelMorianWorkerBuild|TrooperMengskBuild|TychusTerranBuild)/.test(
    ability.ability_id || '',
  );
}
...
function classifyProductionOption({ option, type, entry, entryIds, context }) {
  ...
  const isWorkerBuild = workerBuilderUnitIds.has(option.producer_unit_id || '');
...
function classifyAbility({ ability, entry, type, context }) {
  ...
  if (
    ...
    !context.identity.buildingTargetIds.has(rawCommandUnit)
  ) {
    status = 'excluded';
  ...
  if (
    ...
    !faceMatchesUnitTarget(ability.face || '', new Set([...context.identity.unitTargetIds, ...context.identity.buildingTargetIds]))
  ) {
    status = 'excluded';
...
function classifyTextForCommander({ context, textParts, raw }) {
  ...
  const requirementReferenceText = (raw?.requirements || [])
    .flatMap((requirement) => requirement.referenced_ids || [])
    .join(' ');
  const matchedOtherTokens = context.otherCommanderTokens.filter((token) => searchText.includes(token));
  const matchedOtherOwners = findOtherCommanderOwnerMatches(removeNeutralOwnerTokens(searchText), context.commander);
  const matchedOtherRequirementOwners = findOtherCommanderOwnerMatches(
    removeNeutralOwnerTokens(requirementReferenceText),
    context.commander,
  );
  const reasons = [];
  let status = 'accepted';
  ...
  if (matchedOtherOwners.length) {
    status = 'excluded';
    reasons.push(`other_commander_owner:${matchedOtherOwners.map((hit) => `${hit.owner}:${hit.label}`).join(',')}`);
  }

  if (matchedOtherRequirementOwners.length) {
    status = 'excluded';
    reasons.push(
      `requirement_reference_belongs_to_other_commander:${matchedOtherRequirementOwners
...
function findOtherCommanderOwnerMatches(searchText, currentCommander) {
  return commanderOwnerRules
    .filter((rule) => rule.owner !== currentCommander && rule.pattern.test(searchText))
    .map((rule) => ({ owner: rule.owner, label: rule.label }));
}

function removeNeutralOwnerTokens(searchText) {
  let result = searchText;
  for (const token of neutralOwnerTokens) {
...
function buildCommanderIdentity({ heroes, units, buildings, roster, otherTechEntries }) {
  ...
  const idsByGroup = {};
  const primaryIdsByGroup = {};
  const canonicalIdByAnyId = new Map();
  const buildingCanonicalIdByAnyId = new Map();
  const nameByCanonicalId = new Map();

  for (const [groupName, entries] of Object.entries(groups)) {
    idsByGroup[groupName] = [];
    primaryIdsByGroup[groupName] = new Set();
    for (const entry of entries || []) {
      const canonicalId = entry.id || entry.unit_id || '';
      if (canonicalId && entry.name) {
        nameByCanonicalId.set(canonicalId, entry.name);
      }
      for (const primaryId of unique([entry.id, entry.unit_id].filter(Boolean))) {
        primaryIdsByGroup[groupName].add(primaryId);
      }
      for (const id of getEntryIds(entry)) {
        allIds.add(id);
        idsByGroup[groupName].push(id);
        if (canonicalId && !canonicalIdByAnyId.has(id)) {
          canonicalIdByAnyId.set(id, canonicalId);
        }
        if (groupName === 'buildings' && canonicalId && !buildingCanonicalIdByAnyId.has(id)) {
          buildingCanonicalIdByAnyId.set(id, canonicalId);
        }
      }
    }
...
function buildBuildingTargetIdSet(buildings) {
  const ids = new Set();
...
function renderMarkdown(data) {
  ...
  lines.push('# 人族指挥官完整闭包');
  lines.push('');
  lines.push('生成日期：2026-06-03');
  lines.push('');
  lines.push('本文件由 `scripts/sc2/export-terran-commander-closure.mjs` 从官方数据生成。归属判定先读取 `游戏数据/官方合作指挥官/commanders/<Commander>/`，再用 `游戏数据/官方SC2原始文本镜像/` 补 `AbilData / EffectData / RequirementData` 摘要。');
  lines.push('');
  ...
  lines.push('## 总览');
  lines.push('');
  lines.push('| 指挥官 | 兵种 | buildings.json 建筑 | 劳工建造命令 | 正向单位技能 | 正向建筑技能 | 排除/复核项 |');
  lines.push('|---|---:|---:|---:|---:|---:|---:|');
  for (const commander of data.commanders) {
    lines.push(
      `| ${md(commander.zh_name)} / ${md(commander.commander)} | ${commander.counts.units} | ${commander.counts.buildings} | ${commander.counts.worker_build_commands} | ${commander.counts.accepted_unit_abilities} | ${commander.counts.accepted_building_abilities} | ${commander.counts.excluded_or_review_items} |`,
    );
  }
  lines.push('');
...
function renderCommanderMarkdown(lines, commander) {
  ...
  lines.push('### official buildings.json 建筑建造闭包');
  lines.push('');
  lines.push('本节只统计已经进入 `buildings.json` 的官方建筑条目；重工厂、星港、军械库、精炼厂等功能性 tech building 可能只出现在下面的劳工建造命令闭包里。');
  lines.push('');
  ...
  lines.push('### 劳工实际建造命令闭包');
  lines.push('');
  lines.push('本节来自官方 command card 与 `AbilData`，用于回答“SCV/劳工/冲锋队实际能点哪些建造按钮”。`归属` 表示目标是否也进入当前指挥官 official JSON 名册；`raw-only` 仍可作为功能性前置建筑，但不能替代 official buildings.json 归属。');
  lines.push('');
  lines.push('| 劳工 | 建造链 | Requirement | 归属 | Raw 摘要 |');
  lines.push('|---|---|---|---|---|');
  for (const command of commander.worker_build_commands || []) {
    lines.push(
      `| ${md(formatNamedId(command.worker_id, command.worker_name))} | ${md(renderWorkerBuildCommand(command))} | ${md(command.requirements || '-')} | ${md(renderWorkerBuildOwnership(command))} | ${md(renderRawSummary(command.raw))} |`,
    );
  }
  if (!(commander.worker_build_commands || []).length) {
...
function renderWorkerBuildCommand(command) {
  const target = command.target_canonical_id || command.target_unit_id || command.face || '?';
  const rawTarget =
    command.target_unit_id && command.target_canonical_id && command.target_unit_id !== command.target_canonical_id
      ? ` (raw ${command.target_unit_id})`
      : '';
  return `${command.worker_id || '?'} -> ${command.abil_cmd || '?'} -> ${target}${rawTarget}`;
}

function renderWorkerBuildOwnership(command) {
  const flags = [];
  if (command.target_in_official_buildings) flags.push('buildings.json');
  if (command.target_in_official_units) flags.push('units.json');
  if (command.target_in_official_roster) flags.push('roster.json');
  return flags.length ? flags.join('/') : 'raw-only';
}
