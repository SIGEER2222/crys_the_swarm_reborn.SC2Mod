param(
    ...
    [string]$SummaryPath = "references\official-raynor-import-summary.tsv"
)
...
$seedIds = @(
    "Raynor", "RaynorCommando", "CoopCasterRaynor", "HyperionVoidCoop", "DuskWing",
    "VoidCoopSummonHyperion", "BansheeAirstrike", "CalldownMULE", "RaynorCommander",
    "CommandCenter", "CommandCenterFlying", "SCV", "Marine", "Marauder", "Medic", "Firebat", "Vulture",
    "SiegeTank", "SiegeTankSieged", "VikingFighter", "VikingAssault", "Banshee", "Battlecruiser",
    "SupplyDepot", "SupplyDepotLowered", "Refinery", "Barracks", "BarracksFlying", "BarracksTechLab",
    "Factory", "FactoryFlying", "FactoryTechLab", "Starport", "StarportFlying", "StarportTechLab",
    "EngineeringBay", "Armory", "FusionCore", "Bunker", "MissileTurret", "OrbitalCommand",
    "CommanderPrestigeRaynorBio", "CommanderPrestigeRaynorMechAfterburners", "CommanderPrestigeRaynorAir",
    "RaynorBansheeAirstrike", "RaynorCommanderHyperionAdvancedTargetingSystems",
    "RaynorCommanderHyperionYamatoCooldown", "RaynorCommanderTerranInfantryWeaponRange",
    "RaynorCommanderTerranWeaponAttackSpeed", "RaynorCommanderMechCostReduction",
    "RaynorUnlockBattlecruiser", "RaynorCommanderStimUpgrade", "RaynorFirebatMedicRange",
    "MasteryRaynorStimDuration", "MasteryRaynorMechAttackSpeed", "MasteryRaynorHyperionCooldown",
    "MasteryRaynorDuskWingCooldown", "MasteryRaynorDropPodHaste", "MasteryRaynorStartingSupply"
)

$seedPattern = "^(Raynor|CommanderPrestigeRaynor|MasteryRaynor|HaveMasteryRaynor|HaveRaynor|Hyperion|DuskWing|BansheeAirstrike|VoidCoopSummonHyperion)"
$seedPatternEnd = "(Raynor|VoidCoop)$"
...
foreach ($id in @($selected)) {
    if ($idToSourceFile[$id] -eq "commanderdata.xml" -and $id -ne "Raynor") {
...
foreach ($file in ($sourceDocs.Keys | Sort-Object)) {
    ...
    foreach ($node in $currentSourceDoc.Catalog.ChildNodes) {
        ...
        if ($file -eq "commanderdata.xml" -and $id -ne "Raynor") { continue }
...
Write-Host "Imported $($summaryRows.Count - 1) official Raynor-related catalog objects."
