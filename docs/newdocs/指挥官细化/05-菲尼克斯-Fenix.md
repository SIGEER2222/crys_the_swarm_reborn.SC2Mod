...
口径：英雄人格绑定到普通单位变体，必须和 roster、单位替换、死亡回收一起验证。
...
| 对象 | 按钮/Face | 显示名 | AbilityCmd | Requirement | 说明 |
|---|---|---|---|---|---|
| 使徒 | `MoveHoldPosition` | 原地防御 | `move,HoldPos` | - | 命令选中的单位待在原地，并攻击射程内的敌方目标。接受命令的单位不会对敌人进行追击或移向敌人与其交战。 |
| 使徒 | `MovePatrol` | 巡逻 | `move,Patrol` | - | 命令选中的单位在当前位置与目标区域间进行巡逻。巡逻的单位会对敌人发起攻击或移向附近的敌人与其交战。 |
...
| 保护者 | `MoveHoldPosition` | 原地防御 | `move,HoldPos` | - | 命令选中的单位待在原地，并攻击射程内的敌方目标。接受命令的单位不会对敌人进行追击或移向敌人与其交战。 |
| 保护者 | `MovePatrol` | 巡逻 | `move,Patrol` | - | 命令选中的单位在当前位置与目标区域间进行巡逻。巡逻的单位会对敌人发起攻击或移向附近的敌人与其交战。 |
...
| 名称 | Catalog ID | 解析 Unit | 属性 | 费用/人口/生命 | 备注 |
|---|---|---|---|---|---|
| 不朽者 | `Immortal` | `Immortal, RoboticsBay, RoboticsFacility` | Ground; Armored/Mechanical | 矿:250 气:100 人口字段:-4 生命:200 | 攻击型步战机甲。可以使用屏障吸收伤害。 / 可以对地。 |
...
| 保护者 | `SentryFenix` | `SentryFenix` | Ground; Light/Mechanical/Psionic | 矿:50 气:100 人口字段:-2 生命:40 | 净化者阵营 / 机械支援单位。可以使用防护场和相位模式。 / 可以对地和对空。 |
| 巨像 | `ColossusPurifier` | `Colossus, ColossusPurifier, RoboticsBay, RoboticsFacility` | Air/Ground; Armored/Massive/Mechanical | 矿:300 气:200 人口字段:-6 生命:250 | 步战机器人，装备强大的范围攻击武器。能够攀越悬崖。可升级武器使其攻击能在地面上留下火焰，对敌人造成持续性伤害。 / 可以对地。 |
| 哨兵 | `ZealotPurifier` | `Zealot, ZealotPurifier` | Ground; Biological/Light | 矿:100 气:- 人口字段:-2 生命:100 | 强大的近战战士。升级后可使用冲锋和重构技能。 / 可以对地。 |
| 使徒 | `Adept` | `Adept` | Ground; Biological/Light | 矿:125 气:25 人口字段:-2 生命:70 | - |
...
[XM_DBG][INFO][ROSTER_LOAD] commander=Fenix stage=power_fusion units=8 buildings=4 result=ok
