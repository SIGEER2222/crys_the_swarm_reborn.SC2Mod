...
本文件按 `docs/newdocs/模块拆分` 的 11 个模块整理 泰凯斯。依据 `游戏数据/官方合作指挥官/commanders/Tychus/` 的当前 JSON 生成；具体 Ability、Behavior、Weapon、Actor、Effect、Requirement 闭包仍需继续追 `游戏数据/官方SC2原始文本镜像/` 或实机 `[XM_DBG]` 日志。

## 链路提醒

- 泰凯斯是人族里 `heroes.json=9` 的特殊结构，主要闭包在不法之徒、装备研究和少量专属建筑上，不应按传统兵营/重工/星港兵种树理解。
- `Marauder` 继承到的 `BarracksTrainNova,Train2` 是 Nova 突击手共享训练候选，本轮闭包按 `other_commander_token:Nova` 排除；泰凯斯主链应看 `TychusBarracksTrain` / `TychusFactoryTrain` / `TychusHeroResearch*` 和 `TychusTerranBuild`。
- 后续实现泰凯斯时优先看 `docs/newdocs/指挥官细化/人族闭包/terran-commander-closure.json` 的 accepted 链路。
