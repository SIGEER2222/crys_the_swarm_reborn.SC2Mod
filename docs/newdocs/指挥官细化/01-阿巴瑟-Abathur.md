...
- 私有科技建筑研究链：`EvolutionChamberAbathur / RoachWarrenAbathur / InfestationPitAbathur / SpireAbathur / GreaterSpireAbathur` 分别挂载 `EvolutionChamberResearchAbathur / RoachWarrenResearchAbathur / InfestationPitResearchAbathur / SpireResearchAbathur`。命令卡不回连共享 `evolutionchamberresearch / RoachWarrenResearch / InfestationPitResearch / SpireResearch`，避免多指挥官加载时共享研究 Ability 覆盖。
...
- 阿巴瑟飞蛇技能/研究链路：`LarvaTrainAbathur,Train13` 生产 `ViperAbathur`，`ViperAbathur` 本体显式挂载 `ViperConsumeStructure`、`Yoink`、`ParasiticBomb`。命令卡闭合为 `ViperConsume -> ViperConsumeStructure,Execute`、`FaceEmbrace -> Yoink,Execute`、`ParasiticBomb -> ParasiticBomb,Execute`；本地 Catalog 补齐对应 `Button / Ability / Effect / Behavior / Missile Unit`，避免按钮能点但能力缺效果。感染深渊私有研究闭合为 `InfestationPitResearchAbathur,Research9 -> ViperImprovedCastRange` 和 `InfestationPitResearchAbathur,Research10 -> ViperAbductImprovedStun`，并接入 `Learn*` 需求、`Have*` 被动显示、`ViperImprovedCastRangePassive`、`ViperAbductImprovedStunPassive`。`ParasiticBomb` 按合作模式覆盖值消耗 125 能量，目标过滤为空中可见非己方/友军/中立/建筑目标；公共 `Viper` 不再作为阿巴瑟飞蛇技能链的正链 owner。注意：`Viper` 统一称为飞蛇；`DisablingCloud/BlindingCloud/ViperConsumption` 不是当前阿巴瑟正链技能，不应挂到 `ViperAbathur`。
...
- 满级/精通兜底：`LibE0EAE146_AbathurRuntime.galaxy` 在初始化时强制 `CommanderLevel=16`、`AbathurCommander=1`，并补齐阿巴瑟 15 级核心升级与六项精通 30 点，避免 Bank 未满级时出现按钮显示但 Requirement 不满足。`MasteryAbathurTechFastBuild` 已补到阿巴瑟私有 `UpgradeTo*Abathur / ZergBuildAbathur / *ResearchAbathur` 入口；`AbathurMorphTimeCostReduced` 已补到 `MorphRoachVileToRavager`，使满级秽型虫变破坏者链也获得突变潜能减时。
...
| 飞蛇   | `Viper`           | `Viper`                                  | Air; Psionic; Unit; Melee                             | 矿:100 气:200 人口:-3 生命:150 护盾:- 能量:200 | 官方文本曾含“蔽目毒云”的通用描述；当前阿巴瑟正链命令卡以寄生弹、吞噬、绑架为准，不把 `DisablingCloud/BlindingCloud` 挂为阿巴瑟飞蛇技能。                                                                                                            |
...
- `RoachWarrenResearchAbathur,Research7` -> `RavagerCorrosiveBileRadiusIncrease`：按钮 `EvolveCorrosiveBileRadiusIncrease`，资源 150/150，时间 90；升级将 `RavagerCorrosiveBileAoeSearch` 和光标半径置为 1.5。
- `RoachWarrenResearchAbathur,Research8` -> `RavagerCorrosiveBileDamageIncrease`：按钮 `EvolveCorrosiveBileDamageIncrease`，资源 200/200，时间 120；升级将 `RavagerCorrosiveBileAoeDamage,Amount` 增加 40。
- `AbathurMorphTimeCostReduced`：官方 XML 减少 `MorphRoachToRavager`、`MutaliskMorphToDevourer`、`MutaliskMorphToGuardian` 的变异时间和部分单位成本；当前 Mod 已额外补到满级正链 `MorphRoachVileToRavager,Train1`，并把 `RavagerAbathur / RavagerAbathurBurrowed` 纳入影响单位，避免秽型虫变破坏者链漏掉突变潜能。
...
| 8  | 新单位：飞蛇   | -                                                                                            | -                                                                                          | 官方等级文本曾含“蔽目毒云”的通用描述；当前阿巴瑟满级正链按飞蛇的寄生弹、吞噬、绑架及对应研究/被动闭合。可以对空。                               |
