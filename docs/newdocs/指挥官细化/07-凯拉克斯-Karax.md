...
口径：建筑自动维修和防御建筑强化归个性化机制，科技/面板消耗归特殊机制。
...
| 激励者 | `MoveHoldPosition` | 原地防御 | `move,HoldPos` | - | 命令选中的单位待在原地，并攻击射程内的敌方目标。接受命令的单位不会对敌人进行追击或移向敌人与其交战。 |
| 激励者 | `MovePatrol` | 巡逻 | `move,Patrol` | - | 命令选中的单位在当前位置与目标区域间进行巡逻。巡逻的单位会对敌人发起攻击或移向附近的敌人与其交战。 |
...
| 哨兵 | `MoveHoldPosition` | 原地防御 | `move,HoldPos` | - | 命令选中的单位待在原地，并攻击射程内的敌方目标。接受命令的单位不会对敌人进行追击或移向敌人与其交战。 |
| 哨兵 | `MovePatrol` | 巡逻 | `move,Patrol` | - | 命令选中的单位在当前位置与目标区域间进行巡逻。巡逻的单位会对敌人发起攻击或移向附近的敌人与其交战。 |
...
| 巨像 | `MoveHoldPosition` | 原地防御 | `move,HoldPos` | - | 命令选中的单位待在原地，并攻击射程内的敌方目标。接受命令的单位不会对敌人进行追击或移向敌人与其交战。 |
| 巨像 | `MovePatrol` | 巡逻 | `move,Patrol` | - | 命令选中的单位在当前位置与目标区域间进行巡逻。巡逻的单位会对敌人发起攻击或移向附近的敌人与其交战。 |
| 巨像 | `CliffWalk` | 悬崖攀越 | `-` | - | 巨像能够攀越悬崖。 |
| 巨像 | `ExtendedThermalLance` | 加长热能射线枪 | `-` | HaveKaraxExtendedThermalLance | 使巨像的射程提高3。 |
| 航母 | `MoveHoldPosition` | 原地防御 | `move,HoldPos` | - | 命令选中的单位待在原地，并攻击射程内的敌方目标。接受命令的单位不会对敌人进行追击或移向敌人与其交战。 |
| 航母 | `MovePatrol` | 巡逻 | `move,Patrol` | - | 命令选中的单位在当前位置与目标区域间进行巡逻。巡逻的单位会对敌人发起攻击或移向附近的敌人与其交战。 |
...
| 名称 | Catalog ID | 解析 Unit | 属性 | 费用/人口/生命 | 备注 |
|---|---|---|---|---|---|
| 太阳锻炉 | `SolarForge` | `SolarForge` | Ground; Armored/Mechanical/Structure | 矿:200 气:200 人口字段:- 生命:500 | 为亚顿之矛提供升级方案。 |
...
| 名称 | Catalog ID | 解析 Unit | 属性 | 费用/人口/生命 | 备注 |
|---|---|---|---|---|---|
| 巨像 | `Colossus` | `Colossus, RoboticsBay, RoboticsFacility` | Air/Ground; Armored/Massive/Mechanical | 矿:300 气:200 人口字段:-6 生命:250 | 步战机器人，装备强大的范围攻击武器。能够攀越悬崖。对空武器可以对其进行攻击。 / 可以对地。 |
| 航母 | `Carrier` | `Carrier, FleetBeacon, Stargate` | Air; Armored/Massive/Mechanical | 矿:350 气:250 人口字段:-6 生命:300 | 星灵的主力战舰。能够制造并发射拦截机攻击敌人。 / 可以对地和对空。 |
...
| 激励者 | `SentryPurifier` | `Sentry, SentryPurifier` | Ground; Light/Mechanical/Psionic | 矿:50 气:100 人口字段:-2 生命:40 | 机械支援单位。能够使用时空光束和相位模式技能。升级后可获得回收技能。 / 可以对地和对空。 |
| 哨兵 | `ZealotPurifier` | `Zealot, ZealotPurifier` | Ground; Biological/Light | 矿:100 气:- 人口字段:-2 生命:100 | 强大的近战战士。升级后可使用冲锋和重构技能。 / 可以对地。 |
| 折跃侦察机 | `Scout` | `Scout, Stargate` | -; - | 矿:250 气:75 人口字段:- 生命:150 | 多功能高速战机。 / 可以对地和对空。 |
...
| 护盾充能器 | `ShieldBattery` | `ShieldBattery` | Ground; Mechanical | 矿:100 气:- 人口字段:- 生命:200 | 为附近的友方单位和建筑恢复护盾。升级后可以使用强固屏障。 |
| 太阳锻炉 | `SolarForge` | `SolarForge` | Ground; Armored/Mechanical/Structure | 矿:200 气:200 人口字段:- 生命:500 | 为亚顿之矛提供升级方案。 |
| 侦察机 | `PhoenixPurifier` | `FleetBeacon, Phoenix, PhoenixPurifier, Stargate` | Ground; Mechanical | 矿:300 气:200 人口字段:- 生命:500 | 空中优势战机。可使用引力光束，升级后还可使用相位护甲技能。 / 可以对空。 |
...
| 侦察机 | `CancelBuilding` | 取消 | `BuildInProgress,Cancel` | - | 取消建造，摧毁尚未建造完成的建筑并返还部分资源。 |
| 侦察机 | `ResearchVoidRaySpeedUpgrade` | 研发熔流旋叶 | `FleetBeaconResearch,Research5` | - | 提高虚空辉光舰的移动速度和移动加速度。 |
...
| 护盾充能器 | `CancelBuilding` | 取消 | `-` | - | 取消建造，摧毁尚未建造完成的建筑并返还部分资源。 |
| 太阳锻炉 | `ResearchSolarEfficiencyLevel3Locked` | 研究太阳能利用率等级3 | `-` | KaraxLevel08 | 该科技将在指挥官等级8时解锁。 |
...
[XM_DBG][INFO][ROSTER_LOAD] commander=Karax stage=power_fusion units=7 buildings=6 result=ok
