...
- 模块：`XMKarax.SC2Mod`
- 单位口径：8（官方 JSON 8，在线补充 5）
- 在线主单位：6，supplemental 单位：2
- 建筑口径：6（官方 JSON 5，在线补充 3）
- 在线主建筑：3，supplemental 建筑：3
- 当前 Mod 运行名册：单位 8，建筑 6，生产链补充建筑 4，顶部面板 face 12
- 当前 Mod 顶部面板未解析 Ability：0（0）
- 当前 Mod 顶部面板未解析 Effect：0（0）
- 当前 Mod 生产链补充建筑：Gateway、RoboticsFacility、Stargate、WarpGate
...
| 官方合作指挥官数据到当前 Mod 的硬缺口为 0 | PASS | total_missing=23, loose_only=23, hard_missing=0 |
| 官方 units.json 中的兵种均进入字段级单位审计 | PASS | 8/8 |
| StarCraft2Coop 页面补充的显式兵种也进入字段级单位审计 | PASS | expected_units=8, audited_units=8, online_added=5 |
...
| 当前 Mod runtime 顶部面板按钮引用的 Effect 链均能解析到 Catalog | PASS | unresolved_effects=0 |
...
| 官方合作指挥官数据到当前 Mod 的硬缺口为 0 | PASS | total_missing=19, loose_only=19, hard_missing=0 |
...
- 当前 Mod 可追踪生产/合体/变形目标：9（CorsairMP、DarkArchon、DarkTemplarShakuras、Oracle、OracleStasisTrap、Stalker、VoidRay、Zealot、ZealotShakuras）
...
| 官方合作指挥官数据到当前 Mod 的硬缺口为 0 | PASS | total_missing=21, loose_only=21, hard_missing=0 |
...
| 字段级单位口径均能映射到当前 Mod 运行名册单位 | PASS | runtime_units=8, missing=0 |
| 当前 Mod 可追踪生产/合体/变形目标覆盖字段级单位口径 | PASS | produced_unit_ids=CorsairMP/DarkArchon/DarkTemplarShakuras/Oracle/OracleStasisTrap/Stalker/VoidRay/Zealot/ZealotShakuras, missing=0 |
| 当前 Mod 未暴露未解释的额外生产/合体/变形单位 | PASS | extra=0 |
| 兵种技能/被动不存在硬缺口或字段不匹配 | PASS | unit_skill_issues=0 |
| 兵种技能/被动不再依赖 global-only 提醒项 | PASS | global_only=0 |
