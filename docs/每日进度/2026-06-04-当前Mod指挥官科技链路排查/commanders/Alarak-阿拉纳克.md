...
| --- | --- | --- | --- | --- | --- |
| 初始基地 / Command Center | `NexusAlarak` | building | `Nexus` | 否 / No | Catalog xmalarak，Instance Alarak，文件 / File `合作指挥官版起义狂潮/Mods/XM/XMAlarak.SC2Mod/Base.SC2Data/GameData/UserData.xml` |
| 初始工人 / Worker | `ProbeAlarak` | unit | `Probe` | 否 / No | Catalog xmalarak，Instance Alarak，文件 / File `合作指挥官版起义狂潮/Mods/XM/XMAlarak.SC2Mod/Base.SC2Data/GameData/UserData.xml` |
| 第二初始单位 / Second Unit | `PylonAlarak` | unit | `Pylon` | 否 / No | Catalog xmalarak，Instance Alarak，文件 / File `合作指挥官版起义狂潮/Mods/XM/XMAlarak.SC2Mod/Base.SC2Data/GameData/UserData.xml` |
...
- 关联 Behavior / Linked behaviors：`AllUnitBehaviorController`
- 可生产/创建 / Produced or created：`WarpPrismPhasingTaldarim`（非本指挥官名册 / not in current commander roster）
- 已隐藏基础按钮 / Hidden basic buttons：6 个（用 `--include-basic` 可展开 / use `--include-basic` to expand）
...
| 1,4 | `CommanderPrestigeAlarakMechBuff` | `-` | 未解析 / Unresolved | - | - | CommanderPrestigeAlarakMech |
| 2,0 | 相位模式 / `PhasingModeTaldarim` | `PhasingModeTaldarim,Execute` | 变形技能 / CAbilMorph | 单位 / Unit:`WarpPrismPhasingTaldarim` | - | - |
