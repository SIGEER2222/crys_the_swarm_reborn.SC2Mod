...
3. Runtime calls to `XMFinal` are wired through `XMTestBench_*`:
   ...
   - Raynor now has a first runtime profile for `full_units`, `level15_units`, `fusion_final_units`, `full_buildings`, and `panel_cost_smoke`.
   - Raynor `panel_cost_smoke` currently logs profile metadata for `VoidCoopSummonHyperion`; it does not claim a real cast because `CoopCasterRaynor` is not imported into the current `XMFinal` dependency chain.
   - The map gives player 1 large resources but does not ignore mineral or vespene costs, so future real panel casts can still verify resource deltas. Only food is ignored for roster display.
   - `Run Smoke` currently runs the current commander through `power_fusion`, `standard_base`, `panel_cost_smoke`, `full_buildings`, and `full_units`, clearing between scenarios and aggregating warnings/errors before reporting `ok`, `warning`, or `error`.
   - non-Raynor roster, tech, transport, special, and personal-mechanic scenarios still log profile-stub warnings until the commander profiles are implemented.
